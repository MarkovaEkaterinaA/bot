...
available_prof_name = ["Разработчик", "Аналитик", "Тестировщик"]
...
async def cmd_prof(message: types.Message, state: FSMContext):
    ...
    await message.answer(f'Привет, {name}, выбери профессию', 
                         reply_markup=make_row_keyboard(available_prof_name))
...
@router.message(ChoiceProfNames.choice_prof_names, F.text.in_(available_prof_name))
async def prof_choise(message: types.Message, state: FSMContext):
    await state.update_data(chosen_prof=message.text.lower())
    await message.answer(f'Спасибо, теперь выбери свой уровень', 
...
async def prof_choisen_incorrectly(message: types.Message):
